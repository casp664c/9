...
namespace Kaninbanker
{
    ...
    public sealed class KaninbankerWorldTour : MonoBehaviour
    {
        ...
        {
            if (FindFirstObjectByType<KaninbankerWorldTour>() != null)
                return;
        ...
        private void EvaluateStage(int score)
        {
            ...
            {
                ShowToast("WORLD TOUR: " + score + "/" + target);
            }
        }
        ...
        private void RecalculateStars()
        {
            ...
            for (int i = 0; i < TotalStages; i++)
                totalStars += PlayerPrefs.GetInt(StageKey(i), 0);
        }
        ...
        {
            if (stage < 0 || stage > unlockedStage || stage >= TotalStages)
                return;
        ...
        private void OnGUI()
        {
            ...
            if (GUI.Button(tab, panelOpen ? "LUK TOUR" : "WORLD TOUR", buttonStyle))
                panelOpen = !panelOpen;
            ...
            Rect panel = new Rect(safe.x + margin, safe.y + safe.height * 0.09f,
            ...
            GUI.Label(new Rect(panel.x + inner, panel.y + panel.height * 0.025f,
                panel.width - inner * 2f, panel.height * 0.075f), "WORLD TOUR", titleStyle);

            int chapter = selectedStage / StagesPerChapter;
            int localStage = selectedStage % StagesPerChapter;
            int target = GetTarget(selectedStage);
            int currentStars = PlayerPrefs.GetInt(StageKey(selectedStage), 0);

            float y = panel.y + panel.height * 0.105f;
            GUI.Label(new Rect(panel.x + inner, y, panel.width - inner * 2f, panel.height * 0.065f),
                "VERDEN " + (chapter + 1) + ": " + ChapterNames[chapter], headingStyle);
            y += panel.height * 0.062f;
            GUI.Label(new Rect(panel.x + inner, y, panel.width - inner * 2f, panel.height * 0.05f),
                ChapterTags[chapter] + "  •  " + totalStars + "/" + (TotalStages * 3) + " STJERNER", smallStyle);
            ...
            for (int i = 0; i < StagesPerChapter; i++)
            {
                ...
                if (GUI.Button(new Rect(panel.x + inner + i * (stageW + stageGap), y, stageW, stageH), label, buttonStyle) && unlocked)
                    SelectStage(stageIndex);
                GUI.backgroundColor = old;
            }

            y += stageH + panel.height * 0.035f;
            DrawRect(new Rect(panel.x + inner, y, panel.width - inner * 2f, panel.height * 0.18f),
                new Color(0.055f, 0.075f, 0.11f, 0.96f));
            GUI.Label(new Rect(panel.x + inner * 1.5f, y + panel.height * 0.015f,
                panel.width - inner * 3f, panel.height * 0.055f),
                "STAGE " + (selectedStage + 1) + "  •  SCOREMÅL " + target, headingStyle);
            GUI.Label(new Rect(panel.x + inner * 1.5f, y + panel.height * 0.075f,
                panel.width - inner * 3f, panel.height * 0.08f),
                "1 stjerne: " + target + "   •   2: " + Mathf.RoundToInt(target * 1.4f) + "   •   3: " + (target * 2) +
            ...
            for (int i = 0; i < Chapters; i++)
            {
                ...
                Rect chapterButton = new Rect(panel.x + inner + col * (chapterW + chapterGap),
                    y + row * (chapterH + chapterGap * 0.7f), chapterW, chapterH);
                Color old = GUI.backgroundColor;
                ...
                if (GUI.Button(chapterButton, chapterUnlocked ? "V" + (i + 1) : "LOCK", buttonStyle) && chapterUnlocked)
                    SelectStage(Mathf.Max(firstStage, Mathf.Min(unlockedStage, firstStage + StagesPerChapter - 1)));
                GUI.backgroundColor = old;
            }

            float footerY = panel.yMax - panel.height * 0.105f;
            GUI.Label(new Rect(panel.x + inner, footerY, panel.width - inner * 2f, panel.height * 0.07f),
                "Vælg en stage, luk World Tour og spil en normal runde. Resultatet tæller automatisk mod den valgte stage.", smallStyle);
        }
